...
```
minIO/
├── docker-compose.yml   # MinIO サービス定義
├── .env                 # 環境変数（認証情報・バケット名）
...
| 変数名 | 説明 | デフォルト値 |
|--------|------|-------------|
| `MINIO_ROOT_USER` | 管理者ユーザー名（S3のAccess Keyに相当） | `minioadmin` |
| `MINIO_ROOT_PASSWORD` | 管理者パスワード（S3のSecret Keyに相当） | 任意に設定 |
| `MINIO_BUCKET_NAME` | 起動時に自動作成するバケット名 | `video-cache` |
...
| ボリューム名 | コンテナ内パス | 用途 |
|-------------|---------------|------|
| minio_minio-data | /data | バケットデータ（全オブジェクト） |
...
```
よくある原因:
- MinIO が healthcheck を通過する前に init が実行された → `docker compose down && docker compose up -d` で再起動
- `.env` の認証情報が不正
